<script>
    ...
    import { info, SOTD } from "./music.js";
    let artwork = info.artwork;
    let moreSecs = 1;
    let { maxPos } = info;
    $: moreSecs = (info.nextMax() - $maxPos) / 1000;
    let buttonHeight;
</script>
...
{#if $fields.current}
    <div class="flex my-4">
        <button
            class="mr-2
        w-full h-full text-xl p-5 
        bg-neutral-500/50 
        touch:active:bg-neutral-500/70 
        mouse:hover:bg-neutral-500/70"
            on:click={$fields.skip}
            >Skip{moreSecs ? ` (+${moreSecs}s)` : ""}</button
        >
        ...
        mouse:hover:bg-submit-700/70"
            ...
            bind:clientHeight={buttonHeight}>Submit</button
        >
    </div>
{:else}
    <div class="my-4 flex" style="height: {buttonHeight}px">
        <div
            class="
    bg-gradient-to-r from-primary2-500/70 to-secondary2-500/40 text-left border grow h-full
    "
        >
            <a href={SOTD.url} title="Listen to {SOTD.name} on SoundCloud"
                ><div class="flex items-center h-full">
                    <img src={$artwork} alt={SOTD.name} class="h-full" />
                    <div class="flex-1 mx-3 text-white">
                        <p>{SOTD.name.split(" - ")[1]}</p>
                        <p class="text-sm ">{SOTD.name.split(" - ")[0]}</p>
                    </div>
                    <svg
